                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            const app = initializeApp({ apiKey: "AIzaSyByHm_Zwo9NGZ3DyHtZ5_wCtHlLXcat23Q", authDomain: "darkwave-auth.firebaseapp.com", projectId: "darkwave-auth" }, "garagebot-auth");
                            const auth = getAuth(app);
                            const result = await signInWithPopup(auth, new GoogleAuthProvider());
                            const idToken = await result.user.getIdToken();
                            const res = await fetch("/api/auth/firebase", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ idToken }) });
                            const data = await res.json();
                            if (!res.ok) throw new Error(data.error || "Sign-in failed");
                            toast({ title: "Welcome!", description: `Signed in as ${data.user?.username || data.user?.firstName || "user"}` });
                            setLocation(data.user?.role === "admin" ? "/command-center" : "/dashboard");
                          } catch (err: any) {
