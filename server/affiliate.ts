...
export function registerAffiliateRoutes(app: Express) {
  app.get("/api/affiliate/dashboard", async (req: Request, res: Response) => {
    ...
    res.json({
      uniqueHash,
      referralLink: `https://garagebot.io/ref/${uniqueHash}`,
      tier: computeTier(0),
  ...
  app.get("/api/affiliate/link", async (req: Request, res: Response) => {
    ...
    res.json({
      uniqueHash,
      links: {
        garagebot: `https://garagebot.io/ref/${uniqueHash}`,
  ...
  app.post("/api/affiliate/track", async (req: Request, res: Response) => {
    ...
    res.json({ success: true, message: "Referral tracked", app: "garagebot" });
  });
  ...
  console.log("[AFFILIATE] GarageBot affiliate routes registered");
}
