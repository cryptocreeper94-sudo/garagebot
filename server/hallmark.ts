...
export function generateHallmark(data: {
  ...
  const record: HallmarkRecord = {
    ...
    verificationUrl: `https://garagebot.io/api/hallmark/${thId}/verify`,
...
export function seedGenesisHallmark(): HallmarkRecord {
  ...
  return generateHallmark({
    appId: "garagebot-genesis", appName: "GarageBot", productName: "Genesis Block",
    releaseType: "genesis", metadata: {
      ecosystem: "Trust Layer", version: "1.0.0", domain: "garagebot.io",
...
export function createTrustStamp(userId: string|null, category: string, data: Record<string,any>) {
  const payload = { ...data, appContext: "garagebot", timestamp: new Date().toISOString() };
...
export function registerHallmarkRoutes(app: Express) {
  ...
  app.post("/api/hallmark/company/issue", (req: Request, res: Response) => {
    ...
    const h = generateHallmark({ appId: "garagebot", appName: "GarageBot", productName, releaseType: releaseType||"release", metadata });
    res.json({ success: true, hallmark: h });
  });
  console.log("[HALLMARK] GarageBot hallmark routes registered");
}
