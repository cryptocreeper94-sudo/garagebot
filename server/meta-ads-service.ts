...
export async function createInitialCampaigns(): Promise<any[]> {
  ...
  try {
    const fbCampaign = await createFullCampaign({
      ...
      platform: 'facebook',
      dailyBudget: 10,
      adCopy: '🔧 Find the right part for ANY vehicle — cars, trucks, motorcycles, boats, RVs, ATVs & more! GarageBot searches 94 retailers to get you the best price. Try it free at garagebot.io',
  ...
  try {
    const igCampaign = await createFullCampaign({
      ...
      platform: 'instagram',
      dailyBudget: 10,
      adCopy: '🚗 Stop overpaying for parts! GarageBot compares prices from 94 retailers for cars, trucks, boats, motorcycles & everything in between. Free to use! 🔗 garagebot.io #AutoParts #DIYMechanic #GarageBot',
