...
class TwilioService {
  ...
  async sendWelcome(phoneNumber: string, userName: string): Promise<SMSResult> {
    const message = `Welcome to GarageBot, ${userName}! You're all set to search 95 retailers, get AI diagnoses, and manage your vehicles. Reply STOP to opt out. — garagebot.io`;
    return this.sendSMS({ to: phoneNumber, body: message });
  }
